 ...
 * @returns LED事件数组
 */
export function parseLEDEvents(code) {
  const events = [];
  let timestamp = 0;
 ...
 */
export function calculateFinalState(events, width) {
 ...
 * @returns 判题结果
 */
export function judgeEventSequence(code, expectedEvents) {
  try {
 ...
 * @returns 判题结果
 */
export function judgeFinalState(code, expectedState, width) {
  try {
    const parsedEvents = parseLEDEvents(code);
 ...
 * @returns 判题结果
 */
export function judgeLED(code, grader, gridWidth = 8) {
