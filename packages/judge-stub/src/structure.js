 ...
 */
export function checkStructures(code, requiredStructures = []) {
  const results = {};
 ...
 */
export function validateRequiredStructures(code, requiredStructures = []) {
 ...
 */
export function getMissingStructuresMessage(code, requiredStructures = []) {
  const results = checkStructures(code, requiredStructures);
 ...
 */
export function validateStructure(code, requiredStructures = []) {
  ...
  const message = valid
    ? ''
    : `缺少必需的结构：${missing.map((s) => STRUCTURE_NAME_MAP[s] || s).join(', ')}`;
  return {
    valid,
    details,
    missing,
