...
export class IOStrategy implements JudgeStrategy {
  name = 'io';
  ...
  judge(input: JudgeInput): JudgeResult {
    ...
    try {
      ...
      if (expected && typeof expected === 'object' && Array.isArray(expected.cases)) {
        return this.judgeWithCases(expected, actual, args);
      }
  ...
  private judgeWithCases(expected: any, actual: string, args: any): JudgeResult {
    const { cases, match = 'exact', tolerance = 0, pattern } = expected;
    const warnings: string[] = [];

    switch (match) {
      case 'exact': {
        const passed = cases.every((c: any) => actual === c.out);
        return {
          passed,
          message: passed ? 'All test cases passed' : 'Some test cases failed',
          details: passed ? undefined : `Failed ${cases.length - cases.filter((c: any) => actual === c.out).length} out of ${cases.length} test cases`,
          visualization: {
          ...
          metrics: {
            passed: cases.filter((c: any) => actual === c.out).length,
            total: cases.length,
            similarity: cases.filter((c: any) => actual === c.out).length / cases.length,
          },
        };
      }
      ...
      case 'tolerance': {
        ...
        const passed = !Number.isNaN(numTarget) && 
                      !Number.isNaN(numGot) && 
  ...
  private generateTextDiff(expected: string, actual: string): any {
    // 简单的文本差异生成
    ...
    for (let i = 0; i < maxLines; i++) {
      ...
      if (expectedLine === actualLine) {
        ...
        diff.push(
          { type: 'removed', line: expectedLine },
          { type: 'added', line: actualLine }
        );
      }
    }
