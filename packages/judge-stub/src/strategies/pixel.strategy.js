export class PixelStrategy {
  ...
  judge(input) {
    ...
    try {
      ...
      return {
        ...
        message:
          comparison.similarity >= pixelArgs.similarityThreshold
  ...
  extractPixelMatrix(output) {
    // 优先从 artifacts 获取
    if (output.artifacts?.pixelMatrix) {
      return output.artifacts.pixelMatrix;
    }
    // 从 events 中解析像素数据
    if (output.events) {
  ...
  checkDimensions(expected, actual, args) {
  ...
  compareMatrices(expected, actual, args) {
    ...
    if (args.allowScale && (expected.width !== actual.width || expected.height !== actual.height)) {
      ...
      warnings.push(
        `Scaled actual matrix from ${actual.width}x${actual.height} to ${expected.width}x${expected.height}`,
      );
    }
    const { width, height } = expectedMatrix;
    const diffMatrix = Array(height)
      .fill(null)
      .map(() => Array(width).fill(0));
    let diffCount = 0;
  ...
  isPixelDifferent(expected, actual, args) {
  ...
  scaleMatrix(matrix, targetWidth, targetHeight) {
    const { width, height, pixels } = matrix;
    const scaledPixels = Array(targetHeight)
      .fill(null)
    ...
    for (let y = 0; y < targetHeight; y++) {
      for (let x = 0; x < targetWidth; x++) {
        ...
        if (sourceX < width && sourceY < height) {
          scaledPixels[y][x] = this.getPixelValue(pixels, sourceX, sourceY);
        }
      }
    }
    return {
